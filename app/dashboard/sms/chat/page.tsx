...
export default function SmsChatPage() {
    ...
    return (
        <div className="space-y-5 pb-10 relative min-h-[500px]">
            {(activeTab === "leads" ? loading : loadingSms) && <VictoryLoader />}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: 'var(--ls-heading)', color: 'var(--label-primary)' }}>SMS Chats</h1>
                    <p style={{ fontSize: 13, color: 'var(--label-secondary)', marginTop: 2 }}>Real-time engagement across your SMS leads</p>
                </div>
                    ...
                    <div style={{ display: 'flex', background: 'var(--fill-tertiary)', borderRadius: 'var(--radius-md)', padding: 3, gap: 2 }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                color: activeTab === "owners" ? '#EC4899' : 'var(--label-secondary)',
            ...
            <div className="flex lg:hidden items-center gap-2">
                ...
                >
                    ...
                    {(activeFilters.replyStatus.length > 0 || activeFilters.loops.length > 0 || activeFilters.messageStatus.length > 0 || activeFilters.templates.length > 0) && (
                        <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#EC4899', marginLeft: 2 }} />
                    )}
                </button>
                {(activeFilters.replyStatus.length > 0 || activeFilters.loops.length > 0 || activeFilters.messageStatus.length > 0 || activeFilters.templates.length > 0) && (
                    <button onClick={handleResetFilters} style={{ fontSize: 11, fontWeight: 700, color: '#EC4899', background: 'none', border: 'none', cursor: 'default' }}>RESET ALL</button>
                )}
            </div>
                    ...
                    <div className="liquid-card" style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: '#EC4899', color: '#fff', fontSize: 13, fontWeight: 600,
                ...
                <div className="lg:col-span-3 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                        <div className="md:col-span-2 xl:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-3">
                            <MetricCard title="SMS Sent" value={(activeTab === "leads" ? loading : loadingSms) ? "..." : stats.sentCount.toLocaleString()} desc="Total outgoing SMS pulses" icon={Send} />
                            <MetricCard title="Unique Msg Sent" value={(activeTab === "leads" ? loading : loadingSms) ? "..." : stats.uniqueSentCount.toLocaleString()} desc="Unique entities contacted" icon={Users} />
                            <MetricCard title="Total Replies" value={(activeTab === "leads" ? loading : loadingSms) ? "..." : stats.repliedCount.toLocaleString()} desc={`${stats.responseRate}% Response Rate`} icon={MessageSquare} />
                        </div>
                        <div className="liquid-card" style={{ padding: '14px 16px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                ...
                                <StatusBar label="Replied" value={stats.repliedCount} total={stats.uniqueSentCount || 1} color="#EC4899" />
                                {stats.failedCount > 0 && (
                            ...
                            <>
                                {loading ? (
                                    <div style={{ padding: '40px 16px', textAlign: 'center', color: 'var(--label-tertiary)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                                        <RefreshCw style={{ width: 18, height: 18, color: '#EC4899' }} className="animate-spin" />
    ...
    return (
        <div className="liquid-card" style={{ padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                <div style={{ padding: 7, borderRadius: 'var(--radius-sm)', background: 'rgba(236,72,153,0.10)', color: '#EC4899', flexShrink: 0 }}>
    ...
    return (
        <button
            ...
            style={{
                ...
                background: checked ? 'rgba(236,72,153,0.10)' : 'transparent',
                border: `1px solid ${checked ? 'rgba(236,72,153,0.25)' : 'transparent'}`,
        ...
        >
            <span style={{
                ...
                background: checked ? '#EC4899' : 'var(--fill-secondary)',
                border: `1.5px solid ${checked ? '#EC4899' : 'var(--hairline)'}`,
                transition: 'all 120ms',
            }} />
            <span style={{ fontSize: 12, fontWeight: checked ? 600 : 400, color: checked ? '#EC4899' : 'var(--label-secondary)', flex: 1 }}>{label}</span>
        </button>
    );
}
...
function CustomerRow({ lead: leadRaw, onClick }: { lead: any; onClick: () => void }) {
    ...
    return (
        <tr style={{ borderBottom: '1px solid var(--hairline)', cursor: 'pointer', transition: 'background 100ms' }}
            ...
            <td style={{ padding: '10px 16px', textAlign: 'center' }}>
                <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 'var(--radius-full)', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', background: 'rgba(236,72,153,0.10)', color: '#EC4899', border: '1px solid rgba(236,72,153,0.20)' }}>{lead.source_loop}</span>
            </td>
